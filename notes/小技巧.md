...
- [硬件](#硬件)（ [Windows](#windows) & [MacOS](#macos) & [推荐软件](#推荐软件-1) ）
- [软件管理](#软件管理)（ [Windows](#windows-1) - [Win 键](#win-键) - [Powershell](#powershell) - [CMD](#cmd) & [Linux](#linux) & [MacOS](#macos-1) ）
- [信息查看](#信息查看)
...
- [Docker](#docker)
- [IDE/Editor](#ideeditor)（ [VSCode](#vscode) - [推荐插件](#推荐插件-2) & [JetBrain](#jetbrain) - [推荐插件](#推荐插件-3) & [vi/vim](#vivim) & [推荐软件](#推荐软件-6) ）
- [游戏](#游戏)（ [Steam](#steam) & [Epic](#epic) & [Xbox/MS Store](#xboxms-store) ）
- [AI](#ai)（ [Anaconda](#anaconda) & [CUDA](#cuda) & [Pytorch](#pytorch) & [HuggingFace](#huggingface) & [Lmdeploy](#lmdeploy) & [推荐软件](#推荐软件-7) ）
...
- 部分网站 `闪屏`、`黑屏`、`黑块`，比如 `哔哩哔哩`
  ...
  >
  > 4. 若是 `4K 高刷` 闪屏，确认 `HDMI` 和 `DP` 协议等级是否导致压缩，必要时调低 `刷新率` 或者 `色彩`，但会导致 `HDR` 失效
