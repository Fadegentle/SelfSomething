...
- 磁盘 `100%`，[关闭 Windows 优化功能](https://mbd.baidu.com/newspage/data/landingsuper?sid_for_share=&ruk=fKyFBKnbCCTZ8Odmu2UNNg&like_icon_type=2&isBdboxFrom=1&pageType=1&urlext=%7B%22cuid%22%3A%22liHGugi3Sf_58va3liSa80a1HalPuHi6lOvpal8n2iKQ0qqSB%22%7D&context=%7B%22nid%22%3A%22news_9727084658912958973%22,%22sourceFrom%22%3A%22search%22%7D)
  ...
  - `设备管理器-磁盘驱动器-SSD-“启用设备上的写入缓存”` 关闭严重影响固态硬盘速度，但是有时大分块下载关闭可以直写更快，一般情况固态最好都是开启
...
- 建议使用 `VirtualBox`（免费），`win7` 增强工具安装不上的时候 `https://w0.dk/~chlor/vboxguestadditions/`
- 建议使用 `VMware Workstation`（付费），而且经常出网络问题
- IE11 控制台无法使用安装补丁 `IE11-Windows6.1-KB3008923-x64.msu`
...
- 连通测试 `socat TCP-LISTEN:80,reuseaddr,fork SYSTEM:'echo HTTP/1.1 200 OK; echo; echo OK'`
...
# 切换证书服务器为 LetsEncrypt
 ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
 ~/.acme.sh/acme.sh --issue -d fudongcheng.ddns.net --standalone # 会被大陆限制备案所以建议 alpn
 ~/.acme.sh/acme.sh --issue -d fudongcheng.ddns.net --alpn
 ll ~/.acme.sh/fudongcheng.ddns.net_ecc/

 # 安装证书到挂载目录
mkdir -p /opt/nginx/ssl
~/.acme.sh/acme.sh --install-cert -d fudongcheng.ddns.net \
  --key-file       /opt/nginx/ssl/fudongcheng.key \
  --fullchain-file /opt/nginx/ssl/fudongcheng.pem
```
...
```bash
docker exec -it <mysql-container-name> env LANG=C.UTF-8 mysql -u root -p123456
```
...
- 推荐用于 `Markdown`，[目录（ToC）设置](https://markdown-all-in-one.github.io/docs/guide/table-of-contents.html#overview)
  ...
  - 视野等级 `toc.levels`
  - 粘贴图片路径修改 `setting -> Markdown -> Copy Files: Destination -> item: **/* value: /assets/${documentRelativeDirName}/${documentBaseName}/`
